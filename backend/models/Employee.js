...
const employeeSchema = new mongoose.Schema(
  {
    ...
    // Optional - existing employees added directly (no Offer pipeline)
    // have no linked offer at all.
    offer: { type: mongoose.Schema.Types.ObjectId, ref: "Offer" },
    company: { type: mongoose.Schema.Types.ObjectId, ref: "Company", required: true },
    // Carried over from Onboarding.submittedBy at creation time - the Unit
    // Manager who originally proposed/onboarded this hire. Used to scope
    // the Employees section so each Team Lead only sees their own team,
    // the same way Offers/Onboarding are scoped to their submitter.
